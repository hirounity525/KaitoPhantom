...
public class BossBattlePlayerMover : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            //ジャンプボタン押した時
            if (inputProvider2.isJumpButtonDown2)
            {
                startsJump = true;
            }

            //ジャンプボタン離した時
            if (inputProvider2.isJumpButtonUp2)
            {
                endsJump = true;
            }
        }
    ...
    private void FixedUpdate()
    {
        if (!isKockBack)
        {
            ...
            //横方向の移動の入力・正規化
            Vector3 moveVec = Vector3.right * inputProvider2.moveHorizontal2;
        ...
        else if (inputProvider2.moveHorizontal2 < 0)
        {
